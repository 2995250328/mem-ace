...
# Short Stage1-only inter-frame STGS matrix:
#   1) Indoor6 DINOv2+MapAnything memory: one scene, w=0.25.
#   2) Wayspots ACE-FCN-LMC: bears/cubes, w=0.25 and optional w=0.05.
#   3) Cambridge ACE-FCN-LMC local stage1: GreatCourt, build sidecar if needed,
#      then w=0.25 and optional w=0.05.
#
# Intended to be launched inside tmux from /home/xwh/project/ace_depth:
#   bash ace_dinov2_lmc/scripts/launch_interframe_w025_short_matrix_gpu01.sh
...
RUN_ROOT="${RUN_ROOT:-/data/xwh/ace_dinov2_lmc/04_evaluation/interframe_short_wscan/${STAMP}_w025_plus_w005_gpu01}"
LOG_DIR="${RUN_ROOT}/launcher_logs"
mkdir -p "${LOG_DIR}"
SUPERVISOR_LOG="${LOG_DIR}/supervisor_${STAMP}.log"

GPU_INDOOR="${GPU_INDOOR:-0}"
GPU_WAYSPOTS_A="${GPU_WAYSPOTS_A:-0}"
GPU_WAYSPOTS_B="${GPU_WAYSPOTS_B:-1}"
GPU_CAMBRIDGE="${GPU_CAMBRIDGE:-1}"
GPU_CAMBRIDGE_PREP="${GPU_CAMBRIDGE_PREP:-1}"
...
CAMBRIDGE_SIDECAR_DIR="${CAMBRIDGE_SIDECAR_DIR:-${CAMBRIDGE_PREPROCESS_ROOT}/colmap_keyframe_channel_v1_sp_strict}"

ACEFCN_LMC_ITERATIONS="${ACEFCN_LMC_ITERATIONS:-2}"
ACEFCN_TRAINING_BUFFER_SIZE="${ACEFCN_TRAINING_BUFFER_SIZE:-5000000}"
ACEFCN_BUFFER_SIZE_FINAL="${ACEFCN_BUFFER_SIZE_FINAL:-7600000}"
...
CAMBRIDGE_TRAINING_BUFFER_SIZE="${CAMBRIDGE_TRAINING_BUFFER_SIZE:-5000000}"
CAMBRIDGE_BUFFER_SIZE_FINAL="${CAMBRIDGE_BUFFER_SIZE_FINAL:-5000000}"
CAMBRIDGE_BATCH_SIZE="${CAMBRIDGE_BATCH_SIZE:-4096}"
CAMBRIDGE_IMAGE_RESOLUTION="${CAMBRIDGE_IMAGE_RESOLUTION:-518}"
...
purpose: short Stage1-only inter-frame STGS matrix
run_root: ${RUN_ROOT}
timestamp: ${STAMP}
gpus:
  indoor: ${GPU_INDOOR}
...
wayspots:
  ...
  backbone: ACE-FCN-LMC local stage1
  apply_to: s1
  s2_schedule: none
...
run_indoor_one_weight() {
  local w="$1"
  local label="indoor6_${INDOOR_SCENE}_dino_if_w$(safe_w_label "${w}")_s1"
  ...
  run_logged "indoor ${INDOOR_SCENE} ${label}" "${LOG_DIR}/${label}.log" \
    env \
      ...
      SFM_TRACK_INTER_FRAME_APPLY_TO=s1 \
      ACE_G_S2_SCHEDULE=none \
...
run_wayspots_scene_weight() {
  ...
  local tag="${scene}_acefcn_if_w${wlabel}_s1_it${ACEFCN_LMC_ITERATIONS}_buf5m_${STAMP}"
  ...
  run_logged "wayspots ${scene} w=${w}" "${log_file}" \
    conda run --no-capture-output -n "${CONDA_ENV}" python ace_dinov2_lmc/train_ace_dinov2_lmc.py \
      ...
      --experiment_subdir "stage1_local_interframe_short" \
...
find_cambridge_stage1() {
  find "${CAMBRIDGE_SOURCE_RUN_ROOT}/stage1_local_ace_memory_it12" -type f \
    -name "best_K64_it12_ace_fcn_local_stage1.pt" -path "*${CAMBRIDGE_SCENE}*" 2>/dev/null | sort | tail -n 1
}
...
build_cambridge_sidecar_if_needed() {
  ...
  if [[ ! -f "${model_dir}/points3D.bin" ]]; then
    run_logged "cambridge ${CAMBRIDGE_SCENE} build SuperPoint workspace" "${LOG_DIR}/cambridge_${CAMBRIDGE_SCENE}_build_workspace.log" \
      env PYTHONPATH="/data/xwh/SuperPointPretrainedNetwork:${PYTHONPATH:-}" \
  ...
  if [[ ! -f "${sidecar}" ]]; then
    run_logged "cambridge ${CAMBRIDGE_SCENE} build ACE-FCN sidecar" "${LOG_DIR}/cambridge_${CAMBRIDGE_SCENE}_build_sidecar.log" \
...
run_cambridge_weight() {
  ...
  local sidecar="${CAMBRIDGE_SIDECAR_DIR}/keyframe_channel.npz"
  local tag="${CAMBRIDGE_SCENE}_acefcn_if_w${wlabel}_s1_it${CAMBRIDGE_LMC_ITERATIONS}_buf5m_${STAMP}"
  ...
  run_logged "cambridge ${CAMBRIDGE_SCENE} stage1 w=${w}" "${log_file}" \
    conda run --no-capture-output -n "${CONDA_ENV}" python ace_dinov2_lmc/train_ace_dinov2_lmc.py \
      "${scene_root}" "${tag}.pt" \
      --run_name "${tag}" \
      --model_backend ace_fcn_lmc \
