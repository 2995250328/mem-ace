...
# Train and evaluate MuSHRoom baselines on the aligned ACE-format split.
#
# Defaults run all three MuSHRoom Kinect scenes and four methods:
#   ace, dinoace, aceg25, glace
#
# Typical usage:
#   bash ace_dinov2_lmc/scripts/run_mushroom_baselines_all.sh
#
# Useful overrides:
#   SCENES="coffee_room" METHODS="ace dinoace" DRY_RUN=true bash ...
#   GPU_ID=1 RUN_ROOT=/data/xwh/runs/mushroom_baselines bash ...
#   DEVICE=cuda:1 EVAL_DEVICE=cuda:1 RUN_ROOT=/data/xwh/runs/mushroom_baselines bash ...
...
RUN_ROOT="${RUN_ROOT:-${ROOT_DIR}/ace_dinov2_lmc/04_evaluation/mushroom_baselines/$(date +%Y%m%d_%H%M%S)}"
...
DINOACE_TRAINING_BUFFER_SIZE="${DINOACE_TRAINING_BUFFER_SIZE:-4000000}"
...
print_run_config() {
  ...
  printf "GLACE   : buffer=%s spi=%s batch=%s iterations=%s res=%s\n" \
    "${GLACE_TRAINING_BUFFER_SIZE}" "${GLACE_SAMPLES_PER_IMAGE}" "${GLACE_BATCH_SIZE}" "${GLACE_MAX_ITERATIONS}" "${GLACE_IMAGE_RESOLUTION}"
  printf "ACE-G25 : config=%s\n" "${ACEG25_CONFIG}"
  printf "ACE-G25 : overrides iterations=%s max_passes=%s buffer=%s spi=%s batch=%s map_embs=%s buffer_create_batch=%s workers=%s\n" \
...
train_dinoace() {
  local scene="$1" scene_path="$2" method_dir="$3"
  local dataset_name
  dataset_name="$(basename "${MUSHROOM_ACE_ROOT}")"
  local dino_path
  ...
  local model="${exp_root}/${dataset_name}/${scene}/${scene}_dinov2_ep${DINOACE_EPOCHS}_bs${DINOACE_BATCH_SIZE}_model.pt"
  local pose_file="${method_dir}/poses_${scene}_post_train.txt"
  mkdir -p "${method_dir}"
...
train_aceg25() {
  ...
  run_logged_cwd "$scene" "aceg25" "train" "${method_dir}/train.log" "${ACE_G_ROOT}" \
    "${CONDA_RUN[@]}" python -m ace_g.train_single_scene \
      ...
      "${ACEG25_EXTRA_ARGS[@]}" \
      --dataset.rgb_files "${scene_path}/train/rgb/*.png" \
      --dataset.pose_files "${scene_path}/train/poses/*.txt" \
      --dataset.calibration_files "${scene_path}/train/calibration/*.calibration" \
  ...
  run_logged_cwd "$scene" "aceg25" "register" "${method_dir}/register.log" "${ACE_G_ROOT}" \
    "${CONDA_RUN[@]}" python -m ace_g.register_images \
      ...
      --dataset.rgb_files "${scene_path}/test/rgb/*.png" \
      --dataset.calibration_files "${scene_path}/test/calibration/*.calibration" \
      --output_dir "${method_dir}" \
      --session_id "${session}" \
      --hypotheses "${HYPOTHESES}" \
      --device "${EVAL_DEVICE}"
...
for scene in ${SCENES}; do
  scene_path="${MUSHROOM_ACE_ROOT}/${scene}"
