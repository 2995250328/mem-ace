#!/usr/bin/env bash
# Scene2a continuation run:
# resume the existing best checkpoint and optimize only image-level relative
# depth. The regular S1/S2 reprojection loop is skipped.
#
# Run:
#   cd /home/xwh/project/ace_depth
#   conda activate mapanything
#   GPU_ID=0 bash ace_dinov2_lmc/scripts/run_indoor6_scene2a_reldepth_only_resume_gpu0.sh
...
EXPERIMENT_SUBDIR="${EXPERIMENT_SUBDIR:-indoor6_scene2a_reldepth_only_resume}"
RUN_ROOT="${RUN_ROOT:-${REPO_ROOT}/04_evaluation/train_compare/run_logs/reldepth_only_resume_${SCENE}_$(date +%Y%m%d_%H%M%S)}"
...
REL_DEPTH_ONLY_STEPS="${REL_DEPTH_ONLY_STEPS:-2000}"
REL_DEPTH_ONLY_LR="${REL_DEPTH_ONLY_LR:-1e-5}"
REL_DEPTH_ONLY_TRAIN_FUSION="${REL_DEPTH_ONLY_TRAIN_FUSION:-true}"
RESUME_RUN_DIR="${RESUME_RUN_DIR:-${REPO_ROOT}/04_evaluation/train_compare/indoor6_full_baselines_4090_forceglobal_legacy_s1_reldepth_imagelevel_scene2a/indoor6_ace/scene2a/dino_ace_lmc_ace_g/20260608_114224_aceg_fS2cie_global_res518_buf2.6M_F7.7M_K64_it28_ep24_bs10240_spi_s1buf_sp384_onecycle_improved}"
RESUME_META_PATH="${RESUME_META_PATH:-${RESUME_RUN_DIR}/best_checkpoint_meta.json}"
...
require_inputs() {
  ...
  if [ ! -f "$RESUME_META_PATH" ]; then
    echo "ERROR: missing resume metadata: $RESUME_META_PATH" >&2
    return 1
  fi
}
...
main() {
  ...
  local ts output_name log_file result_file resume_checkpoint
  ts="$(date +%Y%m%d_%H%M%S)"
  output_name="${SCENE}_reldepth_only_resume_${ts}.pt"
  resume_checkpoint="$(jq -r '.best_checkpoint_path' "$RESUME_META_PATH")"
  if [ -z "$resume_checkpoint" ] || [ "$resume_checkpoint" = "null" ] || [ ! -f "$resume_checkpoint" ]; then
    echo "ERROR: invalid best_checkpoint_path in $RESUME_META_PATH: $resume_checkpoint" >&2
    return 1
  fi
  ...
  log "Resume          : ${resume_checkpoint}"
  log "Config          : relative-depth-only steps=${REL_DEPTH_ONLY_STEPS}, lr=${REL_DEPTH_ONLY_LR}, fusion=${REL_DEPTH_ONLY_TRAIN_FUSION}, weight=${REL_DEPTH_LOSS_WEIGHT}"
  ...
  "$PYTHON_BIN" "${REPO_ROOT}/train_ace_dinov2_lmc.py" \
    ...
    --resume_meta_path "$RESUME_META_PATH" \
    --use_lmc True \
    --lmc_flow ace_g \
    --lmc_profile legacy \
    ...
    --relative_depth_start_ratio 0.0 \
    ...
    --relative_depth_only_steps "$REL_DEPTH_ONLY_STEPS" \
    --relative_depth_only_lr "$REL_DEPTH_ONLY_LR" \
    --relative_depth_only_train_fusion "$REL_DEPTH_ONLY_TRAIN_FUSION" \
...

  {
    ...
    echo "training_mode=relative_depth_only"
    echo "relative_depth_only_steps=${REL_DEPTH_ONLY_STEPS}"
    echo "relative_depth_only_lr=${REL_DEPTH_ONLY_LR}"
    echo "relative_depth_only_train_fusion=${REL_DEPTH_ONLY_TRAIN_FUSION}"
