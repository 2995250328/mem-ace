#!/usr/bin/env bash
# Scene5 diagnostic run:
# force-global ACE-G, but use the old/legacy S1 loss-step behavior.
#
# This isolates the scene5 regression seen in:
#   indoor6_full_baselines_4090_forceglobal_s1_periter
#
# It keeps the same memory, aux depth, K/iters/resolution/buffer/batch settings,
# but intentionally does NOT pass --s1_loss_step_mode per_iter. With the legacy
# LMC profile this resolves to the old fixed-zero S1 step behavior.
#
# Run:
#   cd /home/xwh/project/ace_depth
#   conda activate mapanything
#   GPU_ID=0 bash ace_dinov2_lmc/scripts/run_indoor6_scene5_dino_forceglobal_legacy_s1_gpu.sh
...
EXPERIMENT_SUBDIR="${EXPERIMENT_SUBDIR:-indoor6_full_baselines_4090_forceglobal_legacy_s1_scene5}"
RUN_ROOT="${RUN_ROOT:-${REPO_ROOT}/04_evaluation/train_compare/run_logs/forceglobal_legacy_s1_${SCENE}_$(date +%Y%m%d_%H%M%S)}"
...
main() {
  ...
  output_name="${SCENE}_forceglobal_legacy_s1_${ts}.pt"
  ...
  log "Config          : force-global, legacy S1 loss-step (no --s1_loss_step_mode), K64 it28 res518 buf=${TRAINING_BUFFER_SIZE}/${BUFFER_SIZE_FINAL}"
  log "Log             : ${log_file}"
  log "Dry run         : ${DRY_RUN}"
