...
RUN_ROOT_BASE="${RUN_ROOT_BASE:-/data/xwh/ace_dinov2_lmc/04_evaluation/pmrf_centered_c0_fresh_20260621_gpu01}"
MEMORY_ROOT="${MEMORY_ROOT:-/data/xwh/ace_dinov2_lmc/04_evaluation/wayspots_ace_fcn_lmc_suite/20260530_181745/memory}"

# Two-GPU schedule. Keep GPU2/3 unused.
WAVES=(
  "wayspots_squarebench:0 wayspots_cubes:1"
  "wayspots_bears:0 wayspots_tendrils:1"
)
...
for wave in "${WAVES[@]}"; do
  echo "[$(date +%Y-%m-%dT%H:%M:%S)] wave begin: ${wave}"
  pids=()
  for item in ${wave}; do
    scene="${item%%:*}"
    gpu="${item##*:}"
  ...
  echo "[$(date +%Y-%m-%dT%H:%M:%S)] wave done: ${wave}"
done
