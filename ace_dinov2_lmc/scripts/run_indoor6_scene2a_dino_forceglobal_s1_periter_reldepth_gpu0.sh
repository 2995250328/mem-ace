...
# force-global ACE-G with per-iteration S1 loss scheduling plus image-level
# relative-depth distillation in S2-G.
#
# Run:
#   cd /home/xwh/project/ace_depth
#   conda activate mapanything
#   GPU_ID=0 bash ace_dinov2_lmc/scripts/run_indoor6_scene2a_dino_forceglobal_s1_periter_reldepth_gpu0.sh
...
EXPERIMENT_SUBDIR="${EXPERIMENT_SUBDIR:-indoor6_full_baselines_4090_forceglobal_s1_periter_reldepth_imagelevel_scene2a}"
RUN_ROOT="${RUN_ROOT:-${REPO_ROOT}/04_evaluation/train_compare/run_logs/forceglobal_s1_periter_reldepth_imagelevel_${SCENE}_$(date +%Y%m%d_%H%M%S)}"
...
main() {
  ...
  output_name="${SCENE}_forceglobal_s1_periter_reldepth_imagelevel_${ts}.pt"
  ...
  log "Config          : force-global, per-iter S1 loss-step, image-level relative-depth S2-G w=${REL_DEPTH_LOSS_WEIGHT}, interval=${REL_DEPTH_IMAGE_STEP_INTERVAL}, K64 it=${LMC_ITERATIONS} ep=${TRAIN_EPOCHS} res518 buf=${TRAINING_BUFFER_SIZE}/${BUFFER_SIZE_FINAL}"
