#!/usr/bin/env bash
# Scene5 old-best reproduction run:
# request global ACE-G, enable old visibility fallback to local, and keep the
# legacy/default S1 loss-step behavior.
#
# The 20260512 best scene5 run had run_config.s1_loss_step_mode=null and was
# launched as global but auto-switched to local by mean_front_ratio < 0.85:
#   lmc_mode=global, auto_by_visibility=True, fallback=local, repro_step=per_iter.
# This script mirrors that entry configuration under the current codebase.
#
# Run:
#   cd /home/xwh/project/ace_depth
#   conda activate mapanything
#   GPU_ID=1 bash ace_dinov2_lmc/scripts/run_indoor6_scene5_dino_oldbest_repro_gpu.sh
...
EXPERIMENT_SUBDIR="${EXPERIMENT_SUBDIR:-indoor6_full_baselines_4090_oldbest_strict_repro_scene5}"
RUN_ROOT="${RUN_ROOT:-${REPO_ROOT}/04_evaluation/train_compare/run_logs/oldbest_strict_repro_${SCENE}_$(date +%Y%m%d_%H%M%S)}"
LOG_ROOT="${RUN_ROOT}/logs"
RESULT_ROOT="${RUN_ROOT}/results"

MEMORY_PATH="${MEMORY_PATH:-${REPO_ROOT}/memory_extraction/04_evaluation/memory_extract/${SCENE}/40v_v0.05_bilinear_bse_ut0.02_noaug_asb_adaptive_pool1.0_rgate4_prepair8_sor_gm_l2/20260508_105011/memory_bse.pt}"
...
main() {
  ...
  output_name="${SCENE}_oldbest_strict_repro_${ts}.pt"
  ...
  log "Config          : strict old-best repro, requested global -> legacy mean visibility fallback local, legacy S1 loss-step, K64 it28 res518 buf=${TRAINING_BUFFER_SIZE}/${BUFFER_SIZE_FINAL}"
  log "Log             : ${log_file}"
  log "Dry run         : ${DRY_RUN}"
  ...
  "$PYTHON_BIN" "${REPO_ROOT}/train_ace_dinov2_lmc.py" \
    ...
    --lmc_visibility_route_metric legacy_mean \
...

  {
    ...
    echo "log=${log_file}"
    echo "requested_lmc_mode=global"
    echo "auto_visibility_fallback=local"
    echo "lmc_visibility_route_metric=legacy_mean"
