...
# Cambridge stage2 matrix for evaluating whether S2 should adapt fusion to GLACE concat.
# Intended launch: two tmux windows, one scene per GPU. Run from /home/xwh/project/ace_depth.
...
RUN_ROOT="${RUN_ROOT:-/data/xwh/ace_dinov2_lmc/04_evaluation/cambridge_stage2_r2_fusion_matrix_20260626_gpu23}"
SCENE="${SCENE:?Set SCENE, e.g. Cambridge_GreatCourt}"
GPU="${GPU:?Set GPU, e.g. 2}"
VARIANTS="${VARIANTS:-frozen_s10m r2_s10m r2_bigbuf16m}"
...
LMC_ITERATIONS_DEFAULT="${LMC_ITERATIONS_DEFAULT:-10}"
...
FUSION_LR_RATIO_DEFAULT="${FUSION_LR_RATIO_DEFAULT:-0.01}"
...
variant_config() {
  local variant="$1"
  FEATURE_SOURCE="stage1_fused"
  FUSION_IN_S2="False"
  FUSION_LR_RATIO="0.0"
  TRAINING_BUFFER_SIZE="10000000"
  BUFFER_SIZE_FINAL="12000000"
  BATCH_SIZE="4096"
  LMC_ITERATIONS="${LMC_ITERATIONS_DEFAULT}"
  EXTRA_R2_FLAGS=()
  NOTE="frozen_stage1_fused_control"
  ...
  case "${variant}" in
    frozen_s10m)
      ...
      BUFFER_SIZE_FINAL="12000000"
      BATCH_SIZE="4096"
      NOTE="current_mainline_control_frozen_fused_buffer"
      ;;
    r2_s10m)
      FEATURE_SOURCE="raw_backbone"
      FUSION_IN_S2="True"
      FUSION_LR_RATIO="${FUSION_LR_RATIO_DEFAULT}"
      TRAINING_BUFFER_SIZE="10000000"
      BUFFER_SIZE_FINAL="12000000"
      BATCH_SIZE="4096"
      EXTRA_R2_FLAGS=(--lmc_warmup_steps 0 --lmc_train_steps 0)
      NOTE="raw_buffer_on_the_fly_trainable_fusion_compressor_frozen"
      ;;
    r2_bigbuf16m)
      FEATURE_SOURCE="raw_backbone"
      FUSION_IN_S2="True"
      ...
      BUFFER_SIZE_FINAL="16000000"
      BATCH_SIZE="8192"
      EXTRA_R2_FLAGS=(--lmc_warmup_steps 0 --lmc_train_steps 0)
      NOTE="r2_plus_glace_like_16m_buffer_batch8192"
      ;;
    *)
...
run_variant() {
  ...
  local variant_subdir="stage2_${variant}_it${LMC_ITERATIONS}_buf${TRAINING_BUFFER_SIZE}_final${BUFFER_SIZE_FINAL}_bs${BATCH_SIZE}"
