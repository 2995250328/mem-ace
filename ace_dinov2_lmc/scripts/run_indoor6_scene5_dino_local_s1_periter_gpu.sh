...
# local ACE-G with S1 loss-step mode set to per_iter.
#
# This tests the GVCS routing conclusion that scene5 should not be forced global.
# It keeps the same memory, aux depth, K/iters/resolution/buffer/batch settings
# as the force-global diagnostics, changing only lmc_mode=local and
# s1_loss_step_mode=per_iter.
#
# Run:
#   cd /home/xwh/project/ace_depth
#   conda activate mapanything
#   GPU_ID=1 bash ace_dinov2_lmc/scripts/run_indoor6_scene5_dino_local_s1_periter_gpu.sh
...
EXPERIMENT_SUBDIR="${EXPERIMENT_SUBDIR:-indoor6_full_baselines_4090_local_s1_periter_scene5}"
RUN_ROOT="${RUN_ROOT:-${REPO_ROOT}/04_evaluation/train_compare/run_logs/local_s1_periter_${SCENE}_$(date +%Y%m%d_%H%M%S)}"
...
main() {
  ...
  output_name="${SCENE}_local_s1_periter_${ts}.pt"
  ...
  log "Config          : local, S1 loss-step per_iter, K64 it28 res518 buf=${TRAINING_BUFFER_SIZE}/${BUFFER_SIZE_FINAL}"
  log "Log             : ${log_file}"
  log "Dry run         : ${DRY_RUN}"
...

  {
    ...
    echo "log=${log_file}"
    echo "s1_loss_step_mode=per_iter"
  } > "$result_file"
