...
OUTPUT_ROOT="${OUTPUT_ROOT:-/data/xwh/ace_dinov2_lmc/04_evaluation/cambridge_glace_lmc/memory}"
...
if [[ ! -s "${TRAIN_ROOT}/${GLACE_FEAT_NAME}" ]]; then
  echo "ERROR: missing GLACE global features: ${TRAIN_ROOT}/${GLACE_FEAT_NAME}" >&2
  exit 2
fi
...
printf "CAMBRIDGE_GLACE_MEMORY_CMD: "
printf "%q " "${CMD[@]}"
printf "\n"
