...
GLACE introduces global-local accelerated coordinate encoding and uses scene-level context to complement local coordinate prediction~\cite{wang2024glace}. Mem-ACE is complementary: it globally compresses pooled point-feature memory into a fixed token representation and injects that representation through a single memory-fusion block. We use GLACE as a compatible downstream SCR architecture in our experiments, rather than as a component of the Mem-ACE definition.
