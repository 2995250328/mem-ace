...
def run_evaluation_lmc(opt):
    """
    Eval ACE-FCN checkpoint; supports LMC (compressor+fusion+memory) same as test_ace_dinov2_lmc.
    Vanilla checkpoints delegate to run_evaluation. Requires opt.encoder_path for LMC ckpts.
    """
    device = torch.device(opt.device)
    head_network_path = Path(opt.network)
    scene_path = Path(opt.scene)
    ...
    if not _is_lmc_checkpoint(checkpoint):
        return run_evaluation(opt)

    _logger.info("[Eval ACE-FCN+LMC] device=%s cuda_available=%s count=%d",
    ...
    compress_dim = lmc_config.get("compress_dim", 256)
    num_layers = lmc_config.get("num_layers", 4)
    pe_normalize_input = bool(lmc_config.get("pe_normalize_input", False))

    network = RegressorACE.create_from_split_state_dict(
        scene_path=scene_path,
        encoder_path=encoder_path,
    )
    network.heads.load_state_dict(
        head_state_dict if "res3_conv1.weight" in head_state_dict
        else {k.replace("heads.", ""): v for k, v in head_state_dict.items() if k.startswith("heads.")}
    )
    network = network.to(device)
    network.eval()
    backbone_feature_dim = int(network.encoder.feature_dim)

    _actual_dev = next(network.parameters()).device
    _logger.info("[Eval] network on %s (requested %s)", _actual_dev, device)
    if device.type == "cuda" and _actual_dev.type != "cuda":
        _logger.error(
            "[Eval] Network on CPU despite CUDA request. CUDA_VISIBLE_DEVICES=%s",
    ...
    if memory_path is not None:
        ...
        _logger.info("[Eval LMC] Loading memory from %s", memory_path)
        ...
        if raw.get("all_scale_tokens") is not None:
        ...
        _logger.info("[Eval LMC] Memory compressed once (cached per test)")
    ...
    testset_loader = DataLoader(testset, shuffle=False, num_workers=6)
    _logger.info("Test images: %d", len(testset))
    ...
    _logger.info("=" * 50)
    _logger.info("EVAL SUMMARY (ACE-FCN+LMC):")
    _logger.info("  Median: %.2f deg, %.2f cm", median_rErr, median_tErr)
    _logger.info(
        "  25cm/5deg: %.2f%% | 10cm/5deg: %.2f%% | 5cm/5deg: %.2f%%",
        pct25_5, pct10_5, pct5,
    )
    _logger.info("  Avg time: %.2f ms | Frames: %d", avg_time * 1000, total_frames)
    _logger.info("=" * 50)
