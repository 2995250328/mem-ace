...
def _build_run_dir(args):
    ...
    # 算法子目录：vanilla / LMC 迭代流 / LMC ACE-G，便于并列对比
    if not args.use_lmc:
        algorithm = "ace_fcn_vanilla"
    elif getattr(args, "lmc_flow", "iterative") == "ace_g":
        algorithm = "ace_fcn_lmc_aceg"
    else:
        algorithm = "ace_fcn_lmc_iter"
...
def _persist_run_metadata(args, run_dir):
    ...
    for k, v in vars(args).items():
        # 跳过 ResultManager 等不可序列化对象，避免 json.dump 报错
        if k == "result_manager":
            continue
        if isinstance(v, Path):
            ...
            try:
                # 仅在值本身可被 JSON 序列化时直接使用，否则退回到字符串表示
                json.dumps(v)
                config[k] = v
            except TypeError:
                config[k] = str(v)
